...
ToolBoxConfig.forEach(({ key: groupKey, components }) => {
  const group = new Map();

  components.forEach(({ key: componentKey }) => {
    const config = {
      Tool: require(`./toolbox/${groupKey}/${componentKey}/${componentKey}Tool`).default,
      SummaryTool: require(`./summarytoolbox/${groupKey}/${componentKey}/${componentKey}Tool`)
        .default,
      Component: require(`./component/${groupKey}/${componentKey}/${componentKey}Component`)
        .default,
